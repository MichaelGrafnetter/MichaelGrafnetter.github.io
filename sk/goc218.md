...
1. Create a new compliance policy in [Intune](https://endpoint.microsoft.com) with these settings:
    ...
    - Checks
      ...
      - AV present
    - Target: All Devices
2. Re-sync the MDM settings on GOC218-PC4.
...
5. Test the compliance policy by signing in from both GOC218-PC1 and GOC218-PC4.

### Task 4.3: Named Locations (Optional)

1. Create a Named Location called HQ and configure it with the public IP address of the GOC218-PC4 VM.
2. Create a Conditional Access Policy that allows access to the Claims X-Ray app only from the HQ.
3. Test the policy on both GOC218-PC4 and your mobile phone.
...
4. Configure kerberos constrained delegation for the proxy by running the following script:
