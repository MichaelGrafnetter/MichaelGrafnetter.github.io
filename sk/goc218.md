...
### Task 1.1: Register a custom domain name

Register a custom domain called `<givenname>.lab.dsinternals.com` and send the DNS TXT verification code to your trainer.

### Task 1.2: Turn on Passwordless

Enable the following passwordless authentication methods in Azure AD:
...
New-MgUserAuthenticationTemporaryAccessPassMethod `
     -UserId 'jnovak@XXXXX.goc218.dsinternals.com' `
     -IsUsableOnce `
     -LifetimeInMinutes 60 | Format-List

$currentTempPass = Get-MgUserAuthenticationTemporaryAccessPassMethod -UserId 'jnovak@XXXXX.goc218.dsinternals.com'
...
### Task 1.3: Disable SMS authentication

Open the legacy MFA portal and disable SMS authentication. Note that this would prevent an attack called SIM Jacking.

### Task 1.4: Assign the Global Admins role to your account

Invite your personal MS account, convert it to a Member, and assign the Global Admins role to this account.

### Task 1.5: Configure a custom list of banned passwords

Set these passwords as banned:

- Contoso
- Gopas

### Task 1.6: Deploy AAD Password Protection in Hybrid Mode

This task is optional.

### Task 1.7: Enable SSPR

- Enable the Self-Service Password Reset feature for all users.
- Check the Authentication methods Activity report.

### Task 1.8: Enable and configure PIM
...
Get-AzureADUser -ObjectId 'michael@mg.goc218.dsinternals.com'
Get-AzureADUser -Filter "UserPrincipalName eq 'michael@mg.goc218.dsinternals.com'"
...
Set-AzureADUser -ObjectId 4da845a9-a36c-44ad-ac0e-ce15110c9a39 -UserPrincipalName 'Henrietta@mg.goc218.dsinternals.com'
...
# Deleting objects
Remove-AzureADUser -ObjectId 'Grady@mg.goc218.dsinternals.com'
...
New-AzureADUser -UserPrincipalName 'test3@mg.goc218.dsinternals.com' `
                -MailNickName 'test' `
                -AccountEnabled $true `
                -DisplayName 'Test3' `
                -PasswordProfile $passwordProfile

Set-AzureADUserPassword -ObjectId 'test3@mg.goc218.dsinternals.com' `
...
New-MgUser -DisplayName 'Josef Novak' `
           -UserPrincipalName 'jnovak@mg.goc218.dsinternals.com' `
           -PasswordProfile $PasswordProfile `
           -AccountEnabled `
           -MailNickName 'JosefNovak' `
           -GivenName Josef `
