...
1. Enable Group Writeback in Azure AD Connect.
2. Run the `Get-ADSyncAADCompanyFeature` cmdlet on GOC218-SYNC and check that `GroupWritebackV2` is `True`.
3. Test the synchronization by creating a group called `Research and Development` in Azure AD and checking its presence in on-prem AD.
...
3. Enable federation on the `<givenname>.lab.dsinternals.com` domain.

### Task 2.10: Roll-over AAD Connect secrets
...
1. Log in as CONTOSO\vance to CONTOSO-PC1.
2. Visit the domain-specific URL of the Azure Portal: https://myapps.microsoft.com/givenname.goc218.dsinternals.com
...
2. Back on CONTOSO-PC1, run the following commands as CONTOSO\vance:
...
3. Create a new GPO called *Hybrid Domain Join* using the Group Policy Management Console (`gpmc.msc`) on GOC218-DC with the following properties:
   ...
   - Settings
     - Computer Configuration &rArr; Policies &rArr; Administrative Templates &rArr; Windows Components &rArr; Device Registration &rArr; **Register domain joined computers as devices** => Enabled
     - Computer Configuration &rArr; Policies &rArr; Administrative Templates &rArr; Windows Components &rArr; MDM &rArr; **Enable automatic MDM enrollment using default Azure AD credentials** => Enabled, User Credential
4. Reboot the CONTOSO-PC2 computer and log in as CONTOSO\vance.
5. Run the `dsregcmd /status` command.
6. Open the Microsoft Edge browser and check that you are automatically logged in.

### Task 3.5: Enable Kerberos Cloud Trust

Follow these [instructions from Microsoft](https://docs.microsoft.com/en-us/azure/active-directory/authentication/howto-authentication-passwordless-security-key-on-premises#install-the-azure-ad-kerberos-powershell-module) on GOC218-DC.

### Task 3.6: Enable Windows Hello for Business

1. Add the following settings to the *Hybrid Domain Join* GPO on GOC218-DC:
   - Computer Configuration &rArr; Policies &rArr; Administrative Templates &rArr; Windows Components &rArr; Windows Hello for Business => **Use Windows Hello for Business** => Enabled
   - Computer Configuration &rArr; Policies &rArr; Administrative Templates &rArr; Windows Components &rArr; Windows Hello for Business => **Use cloud trust for on-premises authentication** => Enabled
   - Computer Configuration &rArr; Policies &rArr; Administrative Templates &rArr; Windows Components &rArr; Windows Hello for Business => **Use PIN Recovery** => Enabled
   - Computer Configuration &rArr; Policies &rArr; Administrative Templates &rArr; Windows Components &rArr; Windows Hello for Business => **Use a hardware security device** => Enabled
2. Reboot the CONTOSO-PC2 computer and log in as CONTOSO\vance in the Basic session mode.
3. Register a PIN code.
4. Run the `dsregcmd /status` command.

### Task 3.7: Join a PC to Azure AD

1. Log in to CONTOSO-PC3 as Admin.
2. Join the computer to AAD using the adelev@givenname.goc218.dsinternals.com account.
3. Reboot the CONTOSO-PC3 computer and log in as Adele Vance in the Basic session mode.
4. Run the `dsregcmd /status` command.
5. Open the Microsoft Edge browser and check that you are automatically logged in.

 ### Task 3.8: Register a PC in Azure AD

1. Log in to CONTOSO-PC4 as Admin.
...
3. Sign into the **My Apps** portal as Adele Vance from CONTOSO-PC2 and verify that you are able to access the Claims X-Ray app.
